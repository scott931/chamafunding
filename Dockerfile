...
# Configure OPCache for production - Less aggressive caching for faster updates
# These settings prioritize freshness over performance to ensure changes are reflected quickly
RUN echo "opcache.enable=1" >> /usr/local/etc/php/conf.d/docker-php-ext-opcache.ini \
    && echo "opcache.validate_timestamps=1" >> /usr/local/etc/php/conf.d/docker-php-ext-opcache.ini \
    && echo "opcache.revalidate_freq=0" >> /usr/local/etc/php/conf.d/docker-php-ext-opcache.ini \
    && echo "opcache.validate_permission=0" >> /usr/local/etc/php/conf.d/docker-php-ext-opcache.ini \
    && echo "opcache.validate_root=0" >> /usr/local/etc/php/conf.d/docker-php-ext-opcache.ini \
