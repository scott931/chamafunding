...
export default function Sidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 shadow-2xl transform transition-transform duration-300 ease-in-out lg:transition-none border-r border-slate-700/50 backdrop-blur-xl flex flex-col ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        {/* Logo Section */}
        <div className="flex items-center justify-between h-20 px-6 border-b border-slate-700/50 bg-slate-800/50 flex-shrink-0">
        ...
        {/* Navigation Links */}
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto custom-scrollbar min-h-0">
          ...
          {isAdminUser && (
            <>
              ...
              <SidebarLink href="/admin/activity-logs" active={isActive('/admin/activity-logs')}>
                <svg className="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* User Section */}
        <div className="px-4 py-5 border-t border-slate-700/50 bg-slate-800/30 backdrop-blur-sm flex-shrink-0">
